    ...
    function _swapRewardTokens() internal virtual {
        ...
        for (uint256 i = 0; i < _rewardTokens.length; i++) {
            ...
            // If the referenceToken is either A B or C, swap rewards against it 
                    ...
                    _swapReward(reward, referenceToken, _rewardBal, 0); 
            } else {
                ...
                if(ratioA <= ratioB && ratioA <= ratioC) {
                    _swapReward(reward, _tokenA, _rewardBal, 0);
                } else if(ratioB <= ratioC) {
                    _swapReward(reward, _tokenB, _rewardBal, 0);
                } else {
                    _swapReward(reward, _tokenC, _rewardBal, 0);
                }
            }
        }
    }
